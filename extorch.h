...
CustomModuleImpl* GoThread(ThreadGC* thgc) {
    ...
    drop->size.y = 24; drop->size.x = 120;
    PopupWindow* pw = (PopupWindow*)GC_alloc(thgc, CType::_PopupC);
    initPopup(thgc, local, pw, PopupAnchor::Anchor_Element, drop, 0, 2);
    initNewDrop(thgc, drop, pw);
    NewElementAddLast(thgc, local, local, drop);
    NewDown* down = (NewDown*)GC_alloc(thgc, CType::_DownC);
    String* str2 = (String*)createString(thgc, "abcde", 5, 1);
    initNewDown(thgc, down);
    NewElementAddLast(thgc, local, pw, down);
    reinitNewDown(thgc, down, str2);
