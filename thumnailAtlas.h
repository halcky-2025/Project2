...
struct GridPageConfig {
    ...
    uint16_t tileSize = 128;            // タイルサイズ（正方形）
    uint16_t contentSize = 124;         // 実際のコンテンツサイズ
...
public:
    ...
    void createPlaceholder() {
        ...
        m_freeList.pop_back();  // rect 0 を除外
        auto& r = m_rects[0];
...
struct ShelfPageConfig {
    uint16_t atlasWidth = 2048;
    uint16_t atlasHeight = 2048;
    uint16_t rowHeight = 160;           // 行の高さ（パディング込み）
    uint16_t contentHeight = 144;       // 実際のコンテンツ高さ
    uint16_t maxContentWidth = 512;     // 最大コンテンツ幅
    uint8_t maxMipLevels = 4;
    uint64_t loadingTimeoutFrames = 300;
    bgfx::TextureFormat::Enum format = bgfx::TextureFormat::RGBA8;
};
...
public:
    ...
    AtlasPageStats& stats() { return stats_; }  // 外部から更新用
...
// FontIdHash を追加（GlyphKeyHash の近くに配置）
struct FontIdHash {
...
//=============================================================================
// FontAtlas: 修正版（fontIdToIndex_ 追加）
//=============================================================================
class FontAtlas {
public:
    struct Config {
    ...
    std::unordered_map<FontId, uint32_t, FontIdHash> fontIdToIndex_;  // 追加
    uint32_t nextFontIndex_ = 1;  // 追加: 0は無効値
